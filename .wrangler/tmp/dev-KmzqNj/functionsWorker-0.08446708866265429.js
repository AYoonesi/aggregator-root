...
// .wrangler/tmp/pages-NTVWJt/functionsWorker-0.08446708866265429.mjs
...
__name2(mergeEntityMaps, "mergeEntityMaps");
...
__name2(parseLimitTiers, "parseLimitTiers");
...
__name2(parseNCRConfig, "parseNCRConfig");
...
var buildRegexes = /* @__PURE__ */ __name2((startChar, char, flags2 = "") => {
...
var getRegexes = /* @__PURE__ */ __name2((xmlVersion = "1.0", asciiOnly = false) => {
  if (asciiOnly) return regexesAscii;
  return xmlVersion === "1.1" ? regexes11 : regexes10;
}, "getRegexes");
var qName = /* @__PURE__ */ __name2((str, { xmlVersion = "1.0", asciiOnly = false } = {}) => getRegexes(xmlVersion, asciiOnly).qName.test(str), "qName");
...
__name(validateEntityName2, "validateEntityName2");
__name2(validateEntityName2, "validateEntityName");
var SCRIPT_ZEROS = [
...
__name2(handleInfinity, "handleInfinity");
...
__name2(assertContext, "assertContext");
...
__name2(extractRawAttributes, "extractRawAttributes");
  ...
  constructor(options, externalEntities) {
    ...
    if (this.options.entityDecoder) {
      ...
      this.entityDecoder = new EntityDecoder({
        ...
        onInputEntity: /* @__PURE__ */ __name2((name, value) => (
...
__name2(findClosingChar, "findClosingChar");
...
__name2(transformTagName, "transformTagName");
...
__name2(stripAttributePrefix, "stripAttributePrefix");
...
var parser = new XMLParser({
  ...
  trimValues: true,
  parseTagValue: false,
  arrayMode: /* @__PURE__ */ __name2((tagName) => ["item", "entry", "link", "category"].includes(tagName), "arrayMode")
});
var FEED_SOURCES = SITE_CONFIG.rssFeeds;
var FALLBACK_POSTS = [
  {
    id: "fallback-1",
...
function calculateReadingTime(rawContent, title2 = "") {
  if (!rawContent && !title2) return { minutes: 1, text: "1 min read" };
...
__name(calculateReadingTime, "calculateReadingTime");
__name2(calculateReadingTime, "calculateReadingTime");
...
__name2(ensureArray, "ensureArray");
...
__name(getCreator, "getCreator");
__name2(getCreator, "getCreator");
function getContent(item) {
  return safeText(
    item["content:encoded"] || item.content || item.description || item.summary || item["encoded"] || item["content"]
  );
}
...
__name(getPostedDate, "getPostedDate");
__name2(getPostedDate, "getPostedDate");
async function fetchFeeds() {
  const feedPromises = FEED_SOURCES.map(async (source) => {
    try {
      ...
      const channel2 = feedData.rss?.channel || feedData.feed || feedData;
      const items = ensureArray(channel2?.item || channel2?.entry || channel2?.items || []);
      return items.map((item, index) => {
        const titleText = cleanText(safeText(item.title) || safeText(item["title"]));
        const link = getLink(item) || source.url;
        const rawDate = getPostedDate(item) || (/* @__PURE__ */ new Date()).toISOString();
        const parsedDate = new Date(rawDate);
        const validDate = isNaN(parsedDate.getTime()) ? /* @__PURE__ */ new Date() : parsedDate;
...
// .wrangler/tmp/bundle-RyqZw5/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-RyqZw5/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.08446708866265429.js.map
